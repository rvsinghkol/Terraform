...
output "instance_publicip" {
  ...
  value = toset([for instance in aws_instance.ec2vm: instance.public_ip])
}
...
output "instance_publicdns" {
  ...
  value = toset([for instance in aws_instance.ec2vm: instance.public_dns])
}

# EC2 Instance Public DNS with TOMAP
output "instance_publicdns2" {
  value = tomap({for az, instance in aws_instance.ec2vm: az => instance.public_dns})
}
